...
const Test = () => {
    ...
    const paidAbleAmount = (e) => {
        setAmount(e.target.value);
        // console.log(e.target.value);
        // setText((amount * 0.10).toFixed(2))
      };
      const calculateTax = (amount) => {
        
        return (amount * 0.10).toFixed(2); // Assuming a 10% tax
      };
    
      const amountNumber = parseFloat(amount) || 0; // Convert amount to a number
      const tax = calculateTax(amountNumber);
      const totalAmount = (amountNumber + parseFloat(tax)).toFixed(2);
      
      const handelPayments=()=>{
        console.log(totalAmount)
        axios.post('http://localhost:5000/payments',{ amount: totalAmount }, { withCredentials: true })
        .then((res)=>{
  ...
  <div class="p-4 sm:p-7 overflow-y-auto">
    ...
    <div class="mt-5 sm:mt-10 grid grid-cols-2 sm:grid-cols-3 gap-5">
      <div>
        ...
        <span class="block text-sm font-medium text-gray-800 dark:text-neutral-200">{amount}</span>
      </div>
  ...
  <input onChange={paidAbleAmount}
        value={amount}
        type="text"
        name="amount"
         class="py-3 px-4 block w-full border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600" 
         placeholder="Amount went to pay"/>
</div>
          </div>
        </li>
        <li class="inline-flex items-center gap-x-2 py-3 px-4 text-sm border text-gray-800 -mt-px first:rounded-t-lg first:mt-0 last:rounded-b-lg dark:border-neutral-700 dark:text-neutral-200">
